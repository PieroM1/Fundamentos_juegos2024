...
void GLS_Program::compileShader(const string& shaderPath, GLint id)
{
	string filecontent = "";
	string line = "";
	...
	if (isCompiled == GL_FALSE) {
		...
		fatalError("Shaders no linkearion " + printf("%s", &(errorLog[0])));
		glDeleteShader(id);
		return;
	}
...
void GLS_Program::linkShader()
{
	...
	if (isLinked == GL_FALSE) {
		...
		fatalError("Shaders no linkearion " + printf("%s", &(infoLog[0])));
